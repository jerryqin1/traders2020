...
namespace Bot {
  ...
  class Communicator {
  public:
    Communicator(AbstractBot & bot, Router::SenderClient & sender, Router::ReceiverClient & receiver);
  ...
  private:
    ...
    AbstractBot & bot_;
    Router::SenderClient & sender_;
    Router::ReceiverClient & receiver_;
    std::thread t_;
...
namespace Manager {
  ...
  class Manager {
  public:
    void register_bot_client( std::string & prefix, trader_id_t trader);
    void run_competitors(std::string & prefix, std::vector<Bot::AbstractBot*>& bots);
    static trader_id_t get_random_trader_id();
  private:
